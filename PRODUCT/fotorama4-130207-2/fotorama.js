...
function fit ($el, measuresToFit, method) {
  ...
  if (measures && (!elData.last ||
      ...
      elData.last.$w !== elData.$wrap)) {

    //

    var width = measures.width,
        height = measures.height,
        ratio = measuresToFit.width__ / measuresToFit.height__,
    ...
    if (biggerRatioFLAG && (fitFLAG || containFLAG) || !biggerRatioFLAG && coverFLAG) {
      width = minMaxLimit(measuresToFit.width__, 0, fitFLAG ? width : Infinity);
      height = Math.round(width / measures.ratio);
    } else if (biggerRatioFLAG && coverFLAG || !biggerRatioFLAG && (fitFLAG || containFLAG)) {
      height = minMaxLimit(measuresToFit.height__, 0, fitFLAG ? height : Infinity);
    ...
    elData.last = {
      ...
      mh: measures.height,
      mr: measures.ratio,
      mfw: measuresToFit.width__,
      mfh: measuresToFit.height__,
      mm: method,
      $w: elData.$wrap
    }
  }
...
//      height,
  ...
  function onMove (e) {
    ...
    if (!touchEnabledFLAG
      ...
      onEnd();
      return;
    } else if (lastEvent.type.charAt(0) !== e.type.charAt(0)) {
      return;
    }
...
jQuery.Fotorama = function ($fotorama, opts) {
  ...
  var that = this,
      ...
      $_wrap = $('<div></div>'),
      $wrap = $('<div class="fotorama__wrap ' + wrapNotReadyClass + '"></div>').appendTo($_wrap),
      ...
      $navWrap = $_wrap.clone($_wrap),
      $nav = $('<div class="fotorama__nav"></div>').appendTo($navWrap),
      $navShaft = $('<div class="fotorama__nav__shaft"></div>').appendTo($nav),
      $lightboxNav = $('<div class="fotorama__lightbox-nav"></div>'),
      $navFrame,
      $navDotFrame = $(),
      $navThumbFrame = $(),
      $lightboxNavFrame = $(),
      ...
      lightboxNavFrameKey = '$lightboxNavFrame',
  ...
  $wrap[lightboxNavFrameKey] = $('<div class="fotorama__lightbox-nav__frame"></div>');
...
//  }
  ...
  function setOptions () {
    ...
    if (opts.lightbox) {
      ...
      frameDraw(size, 'lightboxNav');
      $lightboxNav.insertAfter($anchor);
      $fotorama
          .appendTo($BODY)
          .addClass('fotorama--hidden');

      if (that.fullScreen && !lastOptions.lightbox) {
  ...
  function setStageShaftMinMaxPosAndSnap () {
    stageShaftData.minPos = o_loop ? - Infinity : - getPosByIndex(size - 1, measures[_side_], o_margin, repositionIndex);
    stageShaftData.maxPos = o_loop? Infinity : - getPosByIndex(0, measures[_side_], o_margin, repositionIndex);
    stageShaftData.snap = measures[_side_] + o_margin;
  }
...
//      }
        ...
        if ((!dataFrame.html || type !== 'stage') && dummy && dummy !== src) {
          ...
          if (src && !frameData.html) {
            ...
            $frame
                ...
                .addClass('fotorama__loaded');

            if (!measuresSetFLAG && index === o_startIndex) {
              // Если размеры ещё не определены пытаемся сделать это по первой фотке
              var width = measureIsValid(opts.width) || WIDTH,
                  height = measureIsValid(opts.height) || HEIGHT;
              that.resize({
                width: width,
                ratio: opts.ratio || width / height
              }, 0, true);
            }
          }
      ...
      function loaded() {
        ...
        if (!measuresSetFLAG || (measuresSetFLAG === '*' && index === o_startIndex)) {
          // Если размеры ещё не определены пытаемся сделать это по первой фотке
          that.resize({
            width: measureIsValid(opts.width) || imgMeasures.width,
            ratio: ratio
        ...
        //if (!dataFrame.noImg || type === 'thumb') {
          $img
              .prependTo($frame)
              .wrap('<div class="fotorama__img' + (fullFLAG ? ' fotorama__img--full' : '') +'"></div>');

          if (type === 'stage') {
            var $imgWrap = imgData.$wrap = $img.parent();
            $imgWrap.data().measures = wrapMeasures;
          }
...
//      }
    ...
    eachIndex(indexes, type, function (i, index, dataFrame, $frame, key, frameData) {
      ...
      if (type === 'stage') {
        ...
        if (dataFrame.html) {
          ...
          var $htmlWrap = $html.data().$wrap = $('<div class="fotorama__html-wrap"></div>').append($html);
          $htmlWrap.data().measures = wrapMeasures;
          $frame.append($htmlWrap);
  ...
  function stageFramePosition (indexes) {
    eachIndex(indexes, 'stage', function (i, index, dataFrame, $frame, key, frameData) {
      ...
      $frame
          .css(_pos, o_fade ? 0 : getPosByIndex(index, measures[_side_], o_margin, repositionIndex))
          .css(_pos2, 0)
      ...
      var method = dataFrame.fit || (that.fullScreen ? opts.fit : opts.fit);

      callFit(frameData.$img, measures, method);
      //callFit(frameData.$full, measures, method);

      callFit(frameData.$html, measures);
    });
  }

  function lightboxNavDraw () {
    var $sampleThumb = $('.fotorama__lightbox-nav__frame:eq(1)', $lightboxNav),
        thumbHeight = $sampleThumb.height(),
        measures = {height__: thumbHeight},
        method = 'contain';

    $lightboxNavFrame.each(function () {
      var $this = $(this),
          thisData = $this.data(),
          eq = thisData.eq;


      if (callFit(thisData.$img, measures, method)) return;

      loadImg([eq], 'lightboxNav', measures, method);
    });
  }
  ...
  function thumbsDraw (pos, loadFLAG) {
    ...
    var $sampleThumb = $('.' + navFrameThumbClass + ':eq(1)', $nav),
        ...
        specialMeasures = {width__: thumbImgSide, height__: thumbImgSide},
  ...
  function navUpdate () {
    ...
    if (opts.lightbox) {
      navAppend($lightboxNavFrame, $lightboxNav);
      //$lightboxNavFrame.removeClass(activeClass);
      //that.activeFrame[lightboxNavFrameKey].addClass(activeClass);
    }
  ...
  function setMarginAndGlimpse () {
    o_margin = Math.max(0, numberFromPercent(opts.margin) / 100 * measures[_side_] || numberFromMeasure(opts.margin)) || 0;
  ...
  function extendMeasures (options) {
    ...
    $.extend(measures, {
      ...
      ratio: eval(options.ratio ? String(options.ratio).replace(':', '/') : null)
    });
  }
...
//    }
    ...
    if (!o_fade) {
      slide($stageShaft, {
        pos: - getPosByIndex(dirtyIndex, measures[_side_], o_margin, repositionIndex),
        _pos: _pos,
        overPos: overPos,
        time: time,
        onEnd: onEnd
  ...
  this.requestFullScreen = function () {
    ...
    setTimeout(function () {
      ...
      $BODY.addClass(lightboxFLAG ? 'lightbox' : 'fullscreen');
  ...
  function cancelFullScreen () {
    ...
    if (FULLSCREEN && !lightboxFLAG) {
      fullScreenApi.cancelFullScreen(fotorama);
    }

    $BODY.removeClass('fullscreen lightbox');
    ...
    //opts.fit = options.flexible ? false : options.fit;
...
//      }
  ...
  stageShaftTouchTail = moveOnTouch($stageShaft, {
    ...
    onEnd: function(result) {
      ...
      var index = getIndexByPos(result.newPos, measures[_side_], o_margin, repositionIndex);
      that.show({
        index: index,
        time: result.time,
